...
/**
 * @brief Recursively traverse the subtree, looking for a node whose data matches value.
 */
...
/**
 * @brief Recursively traverse the subtree and add a new node to the appropriate parent.
 */
 ...
 * 
 * @return true if the node was removed, false otherwise (e.g., did not find a matching node).
 */
bool deleteNode(BSTree *tree, int value) {
    ...
    if (current->left == NULL) {
        ...
        if (parent->left == current) {
            ...
            parent->left = current->right; // Parent's left subtree is now current's right subtree.
        } else {
            // The node to be deleted is in parent's right subtree.
            parent->right = current->right; // Parent's right subtree is now current's right subtree.
        }
        // We have saved a pointer to current's right subtree in its parent, it is now safe to free it.
        free(current);
    ...
    if (current->right == NULL) {
        ...
        if (parent->left == current) {
            // The node to be deleted is in parent's left subtree.
            parent->left = current->left; // Parent's left subtree is now current's left subtree.
        } else {
            // The node to be deleted is in parent's right subtree.
            parent->right = current->left; // Parent's right subtree is now current's left subtree.
        }
        // We have saved a pointer to current's left subtree in its parent, it is now safe to free it.
        free(current);
