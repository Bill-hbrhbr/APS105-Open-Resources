...
void deleteBack(LinkedList *list) {
    ...
    // Thanks to the previous if-statement, we know that head->next is non-NULL.
...
int deleteAllMatches(LinkedList *list, int value) {
    ...
    while (deleteFirstMatch(list, value) == true) {
        numDeleted++;
    }
...
int main(void) {
    printf("Enter 0 to test unordered linked lists.\n");
    printf("Enter 1 to test ordered linked lists.\n");
